...
def get_latest_task_id() -> str | None:
    active_tasks = getattr(orchestrator, "active_tasks", {})
    if not active_tasks:
        return None
...
def build_agents(task: dict[str, Any] | None) -> list[dict[str, Any]]:
    agents = [
        {
            "id": "project",
...
def build_timeline(task: dict[str, Any] | None) -> list[dict[str, Any]]:
    if not task:
        return []
    timeline = []
...
def build_audit_trail(task: dict[str, Any] | None) -> list[dict[str, Any]]:
    if not task:
        return []
    audit_trail = []
    for idx, log_line in enumerate(task.get("logs", [])):
    ...
    return [
        {
            "id": "pm",
            "label": "Projects Managed",
            "value": str(projects_managed),
            "delta": "+1",
        },
        {
            "id": "bm",
            "label": "Budget Managed",
            "value": f"${stats.get('total_budget_managed', 0):,.0f}",
            "delta": "+$20,000",
        },
        {"id": "sr", "label": "Success Rate", "value": "96.4%", "delta": "+1.2%"},
...
# ============================================================
# DYNAMIC CREDIT REPORT ENDPOINT
# ============================================================
...
async def analyze_command(input: AnalyzeCommandInput):
    ...
    if parsed["action"] == "initiate":
        result = orchestrator.initiate_swarm(
            project_brief=parsed["project_brief"], budget_usd=parsed["budget_usd"]
        )
        ...
        recommended_workforce = [
            {
                "id": str(agent.get("agent_id", str(i))),
                "name": f"Agent {agent.get('agent_id', str(i))}",
                "role": agent.get("specialization", "Autonomous Specialist"),
                "trustScore": agent.get("trust_score", 0),
                "hourlyRate": agent.get("hourly_rate_usd", 0),
                "status": "Active",
        ...
        return {
            ...
            "reasoning": reasoning,
            "riskHeadline": "Security-critical workload detected. Performance bonds locked for all agents."
    ...
    if parsed["action"] == "evaluate":
        ...
        result = orchestrator.evaluate_and_heal(
            task_id=task_id, deliverable_summary=parsed["deliverable_summary"]
        )
        await manager.broadcast(
            json.dumps(
                {
        ...
        action_success = (
            "SUCCESS" in str(action_taken).upper()
            or "PASS" in str(action_taken).upper()
        )
        return {
            "transcript": command,
            "summaryCards": [
...
async def initiate_swarm(input: InitiateSwarmInput):
    ...
    result = orchestrator.initiate_swarm(
        project_brief=input.project_brief, budget_usd=input.budget_usd
    )
    task_id = result.get("task_id")
    await manager.broadcast(
        json.dumps(
...
async def evaluate_milestone(input: EvaluateMilestoneInput):
    ...
    result = orchestrator.evaluate_and_heal(
        task_id=input.task_id, deliverable_summary=input.deliverable_summary
    )
    await manager.broadcast(
        json.dumps(
            {
...
async def download_blueprint():
    ...
    if latest_task:
        ...
        team_section = (
            "\n".join(
                [
                    f"- {agent.get('agent_id', 'Unknown')}: {agent.get('specialization', 'Unknown')} (Trust Score: {agent.get('trust_score', 'N/A')})"
                    for agent in team
                ]
            )
            if team
            else "No agents recorded."
        )
    else:
        project_name, budget, status, team_section = (
            "No active project",
            0,
            "IDLE",
            "No active workforce.",
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        await websocket.send_text(
            json.dumps(
                {"type": "connected", "message": "SwarmGuard live stream active"}
            )
        )
        while True:
