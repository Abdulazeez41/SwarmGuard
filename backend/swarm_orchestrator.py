...
class SwarmOrchestrator:
    ...
    def _get_live_agent_data_for_id(self, agent_id: str, role_key: str) -> Dict:
        """
        Fetches LIVE data for a specific agent ID and dynamically 
        ...
        try:
            ...
            # Base rates matching TeamBuilder's logic for consistency
            base_rates = {
                "smart_contract_expert": 200, "frontend_developer": 150, 
                "backend_developer": 160, "security_auditor": 220,
                "qa_engineer": 120, "devops_engineer": 170,
                "data_analyst": 140, "ml_engineer": 240,
                "ux_designer": 130, "content_writer": 90,
                "marketing_strategist": 140, "customer_support": 80,
                "project_manager": 160, "financial_analyst": 170,
                "legal_advisor": 200, "research_analyst": 130,
                "generalist": 100
            }
    ...
    def initiate_swarm(self, project_brief: str, budget_usd: float) -> Dict:
        ...
        self.active_tasks[task_id] = {
            ...
            "status": "IN_PROGRESS", 
            "current_milestone": 0,
            "logs": [f"Swarm initialized. Budget locked: ${budget_usd} USDT in sub-escrow."]
        }
        
        for agent in team_plan["team"]:
            self.active_tasks[task_id]["logs"].append(
                f"HIRED: Agent {agent['agent_id']} ({agent['specialization']}) | Truora Score: {agent['trust_score']}/100 | Performance Bond: 5 OKB locked"
            )
    ...
    def evaluate_and_heal(self, task_id: str, deliverable_summary: str) -> Dict:
        ...
        if task["current_milestone"] >= len(task["team"]):
            return {"status": "COMPLETED", "message": "All milestones already verified."}
        ...
        if is_success:
            ...
            log_entry += f"\n⚖️ PENALTY: 5 OKB performance bond forfeited to client escrow."
            ...
            # Derive role_key directly from specialization (e.g., "Smart Contract Expert" -> "smart_contract_expert")
            role_key = current_agent['specialization'].lower().replace(" ", "_")
    ...
    def _find_replacement_agent(self, role_key: str, remaining_budget: float) -> Dict:
        """
        Attempts live CLI search. If it fails, it fetches LIVE, dynamically 
        calculated data for a pre-vetted backup agent ID.
        """
        ...
        # Fallback to pre-vetted backup agents if CLI search fails
        backup_agents = {
        ...
        if "error" not in replacement_data:
            replacement_data["confidence"] = 82
    ...
    def get_swarm_status(self, task_id: str) -> Dict:
        ...
        return {
            ...
            "global_stats": {
                "projects_managed": self.swarm_memory["completed_projects"], 
                "total_budget_managed": self.swarm_memory["total_budget_managed"]
            },
            "full_event_log": "\n".join(task["logs"])
        }